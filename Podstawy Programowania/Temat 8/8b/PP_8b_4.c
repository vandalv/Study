...
i=1;
 ...
 fseek(fp1, 2*sizeof(struct Silnik), SEEK_SET);
 fread(&bufs, sizeof(struct Silnik),1,fp1);
 strcpy(bufs.producent, "Anaconda");
 fseek(fp1,-1*sizeof(struct Silnik), SEEK_CUR);
 fwrite(&bufs,sizeof(struct Silnik),1,fp1);
 fseek(fp1, 0, SEEK_SET);
 i=1;
 while (fread(&bufs,sizeof(struct Silnik),1,fp1))
 ...
 for(i=0;i<n-1;i++)
 {
 fseek(fp1, i*sizeof(struct Silnik), SEEK_SET);
 fread(&bufs1,sizeof(struct Silnik),1,fp1);
 fread(&bufs2,sizeof(struct Silnik),1,fp1);
 if (bufs1.daneSilnika.pojemnoscSkokowa<bufs2.daneSilnika.pojemnoscSkokowa)
 {
 fseek(fp1, i*sizeof(struct Silnik), SEEK_SET);
 fwrite(&bufs2,sizeof(struct Silnik),1,fp1);
 fwrite(&bufs1,sizeof(struct Silnik),1,fp1);
 zam=1;
 }
 }
 } while(zam);
